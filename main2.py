...
# Ejecutamos el algoritmo de iteración de políticas
algo.iteration_policy_alg(grafo,0.2) # 0.01
...
with open('Resultados2.log', 'w+', encoding='utf-8') as f:
  ...
  f.write(f'\nNodos inicializados:\n\n{algo.ip_result["v_nodes"]}\n')
  f.write(f'\nNúmero de iteraciones: {algo.ip_result["num_iterations"]}\n')
  f.write(f'\nPolítica:\n\n{algo.ip_result["policy"]}\n')
  f.write(f'\nTiempo de ejecución: {algo.ip_result["total_time"]} s\n')
        
'''
  f.write(f'\nPolítica final: \n\n{np.reshape(algo.ip_result["table_arrows"][-1][:-1],(grafo.columns,grafo.rows))}\n')
  ...
  for fila in algo.ip_result['table_arrows']:
      f.write(f'{fila}\n')
'''
